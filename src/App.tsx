...
export default function App() {
  ...
  const filteredOpportunities = useMemo(() => {
    return SAMPLE_OPPORTUNITIES.filter((opp) => {
      // 1. Search Query filter (matches title, org, category, field, location, short/full description)
      if (filters.searchQuery) {
        ...
        const matchCategory = opp.category.toLowerCase().includes(query);
        const matchField = opp.field.toLowerCase().includes(query);
        const matchLocation = opp.location.toLowerCase().includes(query);
        const matchShortDesc = opp.shortDescription.toLowerCase().includes(query);
        const matchFullDesc = opp.fullDescription.toLowerCase().includes(query);

        if (!matchTitle && !matchOrg && !matchCategory && !matchField && !matchLocation && !matchShortDesc && !matchFullDesc) {
          return false;
        }
      }
  ...
  // Featured Opportunities (around 6)
        ...
        {currentPage === 'home' && (
          <div className="space-y-12">
            ...
            <CategoryCards
              ...
              labels={{
                Courses: t.categories.Courses,
                Scholarships: t.categories.Scholarships,
                Internships: t.categories.Internships,
                Fellowships: t.categories.Fellowships,
                Grants: t.categories.Grants,
                Competitions: t.categories.Competitions,
              }}
            />
